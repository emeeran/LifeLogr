...
import {
  ...
  Download, AlertTriangle, CheckCircle2, Loader,
  HardDrive, FolderOpen, Database,
...
function openCreateForm() {
  newProvider.value = 'webdav'; showManualGoogleFields.value = false
  resetNewCredentials(); newSchedule.value = ''; newFreqType.value = 'daily'; newTime.value = '03:00'
  showCreate.value = true
}

function timeToCron(time: string, freq: string): string {
  const [h, m] = time.split(':').map(Number)
  const hh = isNaN(h) ? 3 : h; const mm = isNaN(m) ? 0 : m
...
async function createConfig() {
  ...
  await backupApi.createConfig({ provider: newProvider.value, credentials: filtered, schedule_cron: timeToCron(newTime.value, newFreqType.value) })
  showCreate.value = false
...
async function confirmRestore() {
  ...
  try {
    ...
    emit('toast', 'success', `Restored from ${providerLabel(provider)} — ${(r as any).entries_restored ?? 0}e ${(r as any).media_restored ?? 0}m`)
...
async function handleSyncFlush() {
  ...
  finally { flushConfirm.value = false }
}

// ── Auto Backup ──
const autoBackupEnabled = useLocalStorage<boolean>('lifelogr-auto-backup-enabled', false)
const autoBackupPath = useLocalStorage<string>('lifelogr-auto-backup-path', '~/Backups/lifelogr')
const autoBackupFrequency = useLocalStorage<string>('lifelogr-auto-backup-freq', '0 2 * * *')
const autoBackupRetention = useLocalStorage<number>('lifelogr-auto-backup-retention', 10)
const autoBackupConfigId = useLocalStorage<number | null>('lifelogr-auto-backup-config-id', null)
...
async function saveAutoBackup() {
  ...
  } catch (e: any) { emit('toast', 'error', `Failed to save schedule: ${errMsg(e)}`) }
...
onMounted(() => {
  backup.fetchConfigs(); backup.fetchSnapshots(); syncStore.fetchStatus(); loadAutoBackupStatus()
})
</script>

<template>
  <!-- Local archive -->
  <SettingsSection title="Local Backup" :icon="HardDrive" description="Manual archive of your database and media" setting-key="Local backup">
    <SettingRow :icon="Database" label="Local archive"
      description="Download a .tar.gz of your database and media, or restore one.">
      <SButton variant="ghost" :icon="Download" @click="handleBackupDownload">Backup</SButton>
      <SButton variant="ghost" :disabled="importing" @click="handleBackupRestore">
  ...
  <!-- Auto backup -->
  <SettingsSection title="Scheduled Backup" :icon="RefreshCw" description="Automatic local or cloud backups on a schedule" setting-key="Scheduled backup"
    card-class="p-3 space-y-2">
    <div class="flex items-center gap-2.5">
      <ToggleSwitch v-model="autoBackupEnabled" @update:model-value="toggleAutoBackup" />
      <span class="text-[13px] text-text-secondary flex-1">Auto backup</span>
      <span v-if="autoBackupStatus?.backup_scheduled" class="text-[10px] text-green-400 font-medium flex items-center gap-1">
    ...
    <div v-if="autoBackupEnabled" class="space-y-1.5 pl-7">
      <SettingRow label="Destination">
        <select v-model="autoBackupConfigId" class="settings-select flex-1">
          ...
          <option v-for="c in backup.configs" :key="c.id" :value="c.id">{{ providerLabel(c.provider) }}</option>
        </select>
      </SettingRow>
      <template v-if="!autoBackupConfigId">
        <SettingRow label="Folder">
          <input v-model="autoBackupPath" placeholder="~/Backups/lifelogr" class="settings-input flex-1" />
          <SButton v-if="isTauri" variant="ghost" size="xs" :icon="FolderOpen" @click="browseBackupFolder">Browse</SButton>
        </SettingRow>
        <SettingRow label="Keep">
          <input v-model.number="autoBackupRetention" type="number" min="1" max="100" class="settings-input w-16" />
  ...
  <!-- Cloud -->
  <SettingsSection title="Cloud Backup" :icon="Cloud" description="Sync your journal to cloud storage" setting-key="Cloud backup">
    <template #actions>
      <SButton variant="primary" size="xs" :icon="Plus" @click="openCreateForm">Add</SButton>
    </template>
    ...
    <div v-if="showCreate" class="mb-3 p-3 border border-accent/30 rounded-md space-y-2">
      ...
      <div v-if="newProvider === 'google_drive'" class="flex flex-col gap-1.5 py-1">
        <SButton variant="primary" @click="startGoogleDriveAuth">Sign in with Google</SButton>
      ...
      <template v-if="newProvider !== 'google_drive' || showManualGoogleFields">
        <SettingRow v-for="field in currentFields" :key="field.label" :label="field.label">
          <input v-model="newCredentials[field.label.toLowerCase().replace(/\s+/g, '_')]"
            class="settings-input flex-1" :placeholder="field.placeholder" />
        </SettingRow>
      </template>
      <SettingRow label="Time">
    ...
    <div v-for="config in backup.configs" :key="config.id" class="p-2.5 mb-1 last:mb-0 rounded-md border border-border">
      <div class="flex items-center gap-2 mb-1.5">
        <span class="text-[12px] font-medium text-text-primary">{{ providerLabel(config.provider) }}</span>
        <span v-if="config.last_sync_at" class="text-[10px] text-text-muted">{{ new Date(config.last_sync_at).toLocaleDateString() }}</span>
        <SButton variant="ghost" size="xs" :icon="Trash2" title="Delete config" class="ml-auto !text-text-muted hover:!text-danger"
          @click="deleteConfirm = config.id" />
      </div>
      <div class="flex items-center gap-1.5">
        <SButton variant="ghost" size="xs" @click="testConn(config.id)">Test</SButton>
        <SButton variant="accent-soft" size="xs" :disabled="backingUp === config.id" @click="runBackup(config.id)">
          <Loader v-if="backingUp === config.id" :size="11" class="animate-spin" />{{ backingUp === config.id ? '…' : 'Backup' }}
        </SButton>
        <SButton variant="danger-soft" size="xs" :disabled="restoring === config.id"
          @click="restoreConfirm = { configId: config.id, provider: config.provider }">
          <RotateCcw v-if="restoring === config.id" :size="11" class="animate-spin" />{{ restoring === config.id ? '…' : 'Restore' }}
        </SButton>
      </div>
    ...
    <details v-if="backup.snapshots.length" class="mt-2 border-t border-border pt-2">
      ...
      <div class="mt-1 space-y-0.5">
        <div v-for="snap in backup.snapshots.slice(0, 10)" :key="snap.id" class="px-2 py-0.5 rounded text-[11px] flex items-center gap-2">
  ...
  <!-- Sync -->
  <SettingsSection title="Sync Queue" :icon="RefreshCw" description="Manage pending synchronization operations" setting-key="Sync queue">
    ...
    <div class="flex items-center gap-1.5">
      <SButton variant="accent-soft" :disabled="syncPushing" @click="handleSyncPush">
      ...
      <SButton variant="danger-soft" :icon="Trash2" @click="flushConfirm = true">Flush</SButton>
    </div>
  </SettingsSection>
  ...
  <ConfirmDialog v-if="flushConfirm" title="Flush sync queue?"
    message="This permanently discards all pending sync operations. Continue?"
    @confirm="handleSyncFlush" @cancel="flushConfirm = false" />
</template>
