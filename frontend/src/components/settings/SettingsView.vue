  ...
  <div class="flex flex-col h-full">
    ...
    <div class="flex flex-1 min-h-0">
      ...
      <nav class="w-[140px] shrink-0 border-r border-border py-1.5 px-1 space-y-0.5 overflow-y-auto">
        <!-- Search input -->
        <div class="px-1 mb-1">
          <div class="flex items-center gap-1 bg-surface border border-border rounded px-1.5 py-0.5">
            <Search :size="10" class="text-text-muted shrink-0" />
            <input v-model="searchQuery" placeholder="Search settings..."
              class="bg-transparent text-[10px] text-text-primary outline-none w-full placeholder-text-muted" />
        ...
        <div v-if="!filteredTabs.length" class="px-2 py-1 text-[9px] text-text-muted text-center">
