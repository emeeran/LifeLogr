# Diarilinux — Deployment & Packaging Guide

> How to build, deploy, and package Diarilinux for all platforms.
...
```bash
# Build the image
docker build -t diarilinux .

# Run the container
docker run -d \
  -p 8000:8000 \
  -v diarilinux-data:/data \
  -v diarilinux-media:/app/media \
  --name diarilinux \
  diarilinux
```
...
services:
  diarilinux:
    ...
    ports:
      - "8000:8000"
    volumes:
      - diarilinux-data:/data
      - diarilinux-media:/app/media
    environment:
      - APP_ENV=production
...
The project includes `contrib/diarilinux.service`:

```ini
[Unit]
Description=Diarilinux Journal App
After=network.target

[Service]
Type=simple
User=diarilinux
WorkingDirectory=/opt/diarilinux
ExecStart=/opt/diarilinux/backend/.venv/bin/uvicorn app.main:app --host 0.0.0.0 --port 8000
Environment=APP_ENV=production
Environment=SECRET_KEY=your-secret-key
Environment=DATA_DIR=/var/lib/diarilinux
Restart=always
RestartSec=5
...
```bash
# Copy service file
sudo cp contrib/diarilinux.service /etc/systemd/system/

# Create data directory
sudo mkdir -p /var/lib/diarilinux/media
sudo chown diarilinux:diarilinux /var/lib/diarilinux

# Enable and start
sudo systemctl daemon-reload
sudo systemctl enable diarilinux
sudo systemctl start diarilinux

# Check status
sudo systemctl status diarilinux

# View logs
sudo journalctl -u diarilinux -f
```
...
2. **Backend binary**: PyInstaller bundles `backend/` into `diarilinux-backend`
...
| Platform | Path |
|----------|------|
| Linux | `~/.local/share/diarilinux/` |
| Windows | `%APPDATA%\diarilinux\` |
| macOS | `~/Library/Application Support/diarilinux/` |
...
Mobile uses **direct SQLite** via `@capacitor-community/sqlite` — no Python runtime is needed. The `MobileProvider` in `shared/src/mobile.ts` replicates all CRUD operations in TypeScript.
