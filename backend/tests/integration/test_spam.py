...
async def _seed_account_folder(db_session, label="acct", host="imap.example.com"):
    acct = EmailAccount(
        label=label, email_address="me@example.com",
        imap_host=host, imap_port=993, smtp_host="smtp.example.com", smtp_port=587,
        username="me", password_encrypted="x",
    )
    db_session.add(acct)
...
def _msg(folder, account, frm, subject="hi", uid=1, snippet="hello"):
    return EmailMessage(
        account_id=account.id, folder_id=folder.id, uid=uid,
        from_address=frm, from_name=frm.split("@")[0],
        to_addresses=[{"address": "me@example.com", "name": None}],
        subject=subject, snippet=snippet, sent_at=None,
    )
...
class TestSpamScorer:
    ...
    def test_obvious_spam_scores_high(self):
        s = score_message(
            ...
            "promo@freemoney.xyz", "PayPal Security",
    ...
    def test_legit_newsletter_not_flagged(self):
        ...
        s = score_message(
            "Your weekly digest", "news@reputable.com", "The Newsletter",
            "Here are this week's top stories. To unsubscribe, click the link.", None,
        )
        assert s < SPAM_THRESHOLD
...
class TestSpamClassify:
    async def test_contact_allowlist_keeps_ham(self, client, db_session):
        ...
        db_session.add(_msg(folder, acct, "anna@acme.com",
                            subject="WINNER!!!", snippet="claim your prize", uid=1))
    ...
    async def test_blocklist_makes_spam(self, client, db_session):
        acct, folder = await _seed_account_folder(db_session)
        db_session.add(_msg(folder, acct, "promo@spammer.top",
                            subject="deals", snippet="buy now", uid=2))
        await db_session.commit()
        await client.post("/api/v1/email/spam/rules", json={"pattern": "spammer.top", "is_domain": True})
        await client.post("/api/v1/email/spam/rescore")
    ...
    async def test_list_exclude_spam_flag(self, client, db_session):
        ...
        shown = (await client.get("/api/v1/email/messages?exclude_spam=false&spam_only=true")).json()
