...
class TestSyncQueue:
    async def test_enqueue(self, client: AsyncClient):
        r = await client.post("/api/v1/sync/enqueue", json={
            "operation": "create", "entity_type": "entry", "entity_id": 1,
            "payload": {"body": "test"},
        })
    ...
    async def test_get_pending(self, client: AsyncClient):
        await client.post("/api/v1/sync/enqueue", json={
            "operation": "update", "entity_type": "entry", "entity_id": 2,
            "payload": {},
        })
    ...
    async def test_flush(self, client: AsyncClient):
        await client.post("/api/v1/sync/enqueue", json={
            "operation": "delete", "entity_type": "entry", "entity_id": 3,
            "payload": {},
        })
