"""Google OAuth + authenticated REST helper for Calendar & Tasks sync.

Mirrors the Drive provider's token model (``cloud_sync_service.GoogleDriveProvider``)
but generalized for any Google REST API, so ``CalendarSyncService`` and
``TasksSyncService`` share one client. Gmail is **not** handled here — it stays
on its own IMAP connection.
...
def build_auth_url(client_id: str, state: str) -> str:
    """Build the Google consent URL for Calendar+Tasks scopes."""
    params = {
        "client_id": client_id,
