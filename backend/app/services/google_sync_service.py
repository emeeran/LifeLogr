"""Orchestrates two-way Google Calendar + Tasks + Contacts sync for the account.

Loads the single ``GoogleSyncAccount``, builds a ``GoogleAPIClient`` whose
token-refresh callback persists refreshed tokens, then runs Calendar, Tasks,
and Contacts sync — each in its own try/except so one failing service doesn't
abort the others (mirroring ``EmailSyncService.sync_all_accounts``). Never raises.
"""
...
class GoogleSyncService:
    """Runs Calendar + Tasks + Contacts sync for the connected Google account."""
    ...
    async def sync_all(self) -> dict[str, Any]:
        """Sync Calendar + Tasks + Contacts; return a per-service summary. Never raises."""
        ...
        result: dict[str, Any] = {"calendar": None, "tasks": None, "contacts": None}
        errors: list[str] = []
        async with GoogleAPIClient(creds, on_token_refresh=on_refresh) as api:
            ...
            try:
                result["contacts"] = await ContactsSyncService(self.db, account, api).sync()
            except Exception as exc:
                logger.warning("Contacts sync failed", exc_info=True)
                errors.append(f"contacts: {exc}")
                result["contacts"] = {"error": str(exc)}
