"""GoogleSyncAccount ORM model — one Google OAuth connection for Calendar+Tasks+Contacts."""
...
class GoogleSyncAccount(Base):
    """A single Google account connection used for Calendar + Tasks + Contacts sync.

    Holds the encrypted OAuth token (scopes: ``calendar`` + ``tasks`` +
    ``contacts``) plus per-service incremental-sync cursors and state. One row
    per Google account (a singleton in practice — the app is single-user). Gmail
    stays on its own IMAP connection and is not represented here.
    """
    ...
    contacts_enabled: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    ...
    # People API syncToken (NULL → full pull with requestSyncToken).
    contacts_sync_token: Mapped[str | None] = mapped_column(String, nullable=True)
