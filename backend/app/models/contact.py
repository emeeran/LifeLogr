...
Contacts come from four sources, tracked by ``source``:
  * ``manual`` — user-created via the UI
  * ``email``  — auto-extracted from email correspondence
  * ``vcard``  — imported from a .vcf file
  * ``google`` — pulled from Google Contacts (People API two-way sync)
...
``email`` contacts may have their name enriched when empty. ``google`` contacts
are owned by the Contacts sync service (``external_id``/``etag``/``synced_at``
provenance mirror the Calendar/Tasks pattern).
"""
...
class Contact(Base):
    ...
    # "manual" | "email" | "vcard" | "google"
    source: Mapped[str] = mapped_column(String(20), default="manual", nullable=False)
    # Google Contacts sync provenance (external_id = People resourceName).
