...
_COLUMN_MIGRATIONS = [
    ...
    # ── Google Contacts sync provenance on contacts + per-account cursor ──
    ("contacts", "external_id", "ALTER TABLE contacts ADD COLUMN external_id VARCHAR(255)"),
    ("contacts", "etag", "ALTER TABLE contacts ADD COLUMN etag VARCHAR(255)"),
    ("contacts", "synced_at", "ALTER TABLE contacts ADD COLUMN synced_at DATETIME"),
    (
        "google_sync_account",
        "contacts_enabled",
        "ALTER TABLE google_sync_account ADD COLUMN contacts_enabled BOOLEAN NOT NULL DEFAULT 1",
    ),
    (
        "google_sync_account",
        "contacts_sync_token",
        "ALTER TABLE google_sync_account ADD COLUMN contacts_sync_token VARCHAR",
    ),
]
