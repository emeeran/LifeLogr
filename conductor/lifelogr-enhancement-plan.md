...
| F04 | Rebrand to "LifeLogr" + logo | Branding | Config + main.py title | — | P1 |
...
**Logo:**
- Create `assets/lifelogr-logo.svg` — feather pen + Tux-inspired elements
...
- [ ] App renamed to "LifeLogr" across all files
- [ ] SVG logo created and served statically
- [ ] `pyproject.toml` and config updated
- [ ] All SDD docs reference "LifeLogr"
...
**Implementation:**
- **PDF:** `weasyprint` (HTML-to-PDF) with LifeLogr-branded template, embedded media (base64)
...
| Sync model | Centralized cloud vs BYO-Cloud | **BYO-Cloud** (WebDAV/Dropbox/Google Drive) | Users retain total ownership of their data. No LifeLogr-hosted server required. |
...
```
backend/app/
├── main.py                          # Updated: LifeLogr branding, static files, all routers
├── core/
│   ├── __init__.py
